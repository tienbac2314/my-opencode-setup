...
export declare function scopeSearchFilters(scope: Scope, userId: string, appId: string, runId: string): Record<string, string>;
/** Identity params for `add` / `delete_all` at the given scope. */
export declare function scopeWriteParams(scope: Scope, userId: string, appId: string, runId: string): {
    user_id: string;
    app_id?: string;
    run_id?: string;
};
/** Normalize an arbitrary value to a valid Scope (defaults to "project"). */
export declare function asScope(value: unknown): Scope;
/**
 ...
 */
export declare function resolveDefaultScope(settings: Record<string, unknown> | null | undefined): Scope;
/** Guidance injected so the agent uses `global` only when explicitly asked. */
export declare const SCOPE_GUIDANCE = "Memory tools accept an optional `scope`: omit it (or \"project\") for normal queries; use \"session\" to limit to the current run; use \"global\" ONLY when the user explicitly asks to search across all their projects in this workspace.";
