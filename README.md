...
| `codegraph-helper.ts` | auto-discovered file | Requires one CodeGraph attempt before grep/glob fallback | Per-session guard; `.codegraph/` gate; MCP watcher owns index refresh | 5 hook tests plus live status and explore |
